...
const Testimonials = () => {
  ...
  return <section className="py-20 md:py-28 bg-gradient-subtle overflow-hidden">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="overflow-hidden" ref={emblaRef}>
          <div className="flex gap-6 transition-transform duration-500 ease-out">
            {testimonials.map((testimonial, index) => <div key={index} className={`flex-[0_0_100%] min-w-0 sm:flex-[0_0_50%] lg:flex-[0_0_33.333%] ${testimonial.name === "Roberto Lima" ? "lg:flex-[0_0_28%]" : ""}`}>
                <Card className="p-6 shadow-card hover:shadow-elegant transition-all duration-300 h-full border-2 border-primary/30 bg-card/80 backdrop-blur-sm">
                  <div className="flex items-center gap-3 mb-4">
                    {testimonial.image ? <img src={testimonial.image} alt={testimonial.name} className="w-14 h-14 rounded-full object-cover shadow-soft border-2 border-primary/20" /> : <div className="w-14 h-14 rounded-full bg-gradient-primary flex items-center justify-center text-white font-bold text-lg shadow-soft">
                        {testimonial.avatar}
                      </div>}
                    <div>
