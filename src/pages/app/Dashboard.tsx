...
export default function Dashboard() {
  ...
  const loadProfile = async (userId: string) => {
    try {
      const {
        data,
        error
      } = await supabase.from('profiles').select('avatar_url, full_name').eq('id', userId).maybeSingle();
  ...
  const loadDashboardStats = async (userId: string) => {
    try {
      ...
      } = await supabase.from('elderly_profiles').select('id').eq('caregiver_id', userId);
      ...
      const {
        data: tasks
      } = await supabase.from('daily_tasks').select('completed, completed_at').in('elderly_id', elderlyIds);
      ...
      } = await supabase.from('appointments').select('title, appointment_date').in('elderly_id', elderlyIds).gte('appointment_date', new Date().toISOString()).order('appointment_date', {
        ascending: true
      }).limit(1).maybeSingle();
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <CardDescription className="flex items-center gap-2 mt-1">
                    {subscription.plan === 'pro' ? <Badge variant="default" className="gap-1">
                        <Crown className="h-3 w-3" />
                        Plano Pro
                      </Badge> : <Badge variant="secondary">Plano Básico</Badge>}
                  </CardDescription>
                </div>
              </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {stats.recentActivities.length > 0 ? stats.recentActivities.map(activity => <div key={activity.id} className="flex items-start gap-3 pb-3 border-b last:border-0">
        ...
        <Card className="mb-8">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Button onClick={() => navigate('/app/tasks')} className="h-auto py-4 flex-col gap-2">
                <Plus className="h-5 w-5" />
                Nova Tarefa
              </Button>
              <Button onClick={() => navigate('/app/agenda')} variant="outline" className="h-auto py-4 flex-col gap-2">
                <Calendar className="h-5 w-5" />
                Novo Evento
              </Button>
              <Button onClick={() => navigate('/app/history')} variant="outline" className="h-auto py-4 flex-col gap-2">
